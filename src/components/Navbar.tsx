...
export const Navbar: React.FC = () => {
  ...
  return createPortal(
    <>
      ...
      {/* Logo */}
      <div className="gradient-brand text-foreground h-[34px] w-[34px] border border-foreground flex items-center justify-center">
      ...
      <div className="hidden md:flex items-center">
        ...
        >
          ...
          <span className="absolute inset-0 gradient-brand translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out"></span>
        </Link>
        <button 
          onClick={() => {
            if (user) {
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-[3000] flex flex-col animate-in slide-in-from-top duration-300">
          {/* Close header */}
          <div className="gradient-brand flex items-center justify-center py-16 animate-in fade-in duration-500">
            <button
              onClick={() => setIsMobileMenuOpen(false)}
              className="text-foreground text-[11px] font-medium uppercase tracking-wider"
            >
              CLOSE
            </button>
          </div>
      ...
      <div className="md:hidden flex items-center">
        <button 
          ...
          className="relative overflow-hidden bg-background text-foreground h-[34px] px-3 border border-l-0 border-foreground flex items-center justify-center text-[11px] font-medium uppercase leading-none group"
        >
